...
void siftDownScriptV2(std::vector<Block*>& heapList, int i, int n) {
    ...
    int child = 2 * i + 1;
    if(child < n){
        scriptV2.push_back({ -1,  i, -1, "So sánh cha và con trái", StepTypeV2::COMPARE, child });
    ...
    if (child + 1 < n) {
        scriptV2.push_back({ -1, swapIdx, -1, "So sánh với con phải", StepTypeV2::COMPARE, child + 1 });
    ...
    if(swapIdx == i) return;
    scriptV2.push_back({ -1, i, -1, "Đổi chỗ cha con (Max Heap)", StepTypeV2::SWAP_DOWN, swapIdx });
...
void buildMaxHeapScriptV2(std::vector<Block*>& heapList) {
    ...
    for (int i = n / 2 - 1; i >= 0; i--) {
        scriptV2.push_back({ -1, i, -1, "Sifting down", StepTypeV2::TRAVERSE, root});
...
void heapifyScriptV2(std::vector<Block*>& heapList, int i) {
    root = i;
    int parent = (i - 1) / 2;
    scriptV2.push_back({ -1, i, -1, "COMPARE", StepTypeV2::COMPARE, parent});
    if(isMaxHeap && std::stoi(heapList[parent]->getLabel()) < std::stoi(heapList[i]->getLabel())) {
        std::swap(heapList[parent], heapList[i]);
        scriptV2.push_back({ -1, i, -1, "Sifting up", StepTypeV2::SWAP_UP, parent});
...
void insertScriptV2(std::vector<Block*>& heapList, int value) {
    ...
    scriptV2.push_back({ -1, (int)heapList.size() - 1, -1, "Đã tạo xong node!", StepTypeV2::TRAVERSE});
    heapifyScriptV2(heapList, heapList.size() - 1);
}

// Heap
void initVisualization2(sf::RenderWindow& window) {
    ImGui::TextColored(ImVec4(0, 255, 0, 255),"Initialize a heap array:");
    ImGui::Spacing();
    ...
    if (ImGui::Button("Max Heap", ImVec2(120, 40)) && !isMaxHeap && checkFinishedV2()) {
        isMaxHeap = true;
        std::string data = "";
    ...
    if (ImGui::Button("Min Heap", ImVec2(120, 40)) && isMaxHeap && checkFinishedV2()) {
        isMaxHeap = false;
        std::string data = "";
        for(auto node:heapList) {
            data += node->getLabel();
    ...
    if (temp == false && checkFinishedV2() && ImGui::Button("Random", ImVec2(100.0f, 30)))
    {
        int n = rand() % 8;
        std::string data = "";
        for(int i = 0; i < n; i++){
...
void insertVisualization2(sf::RenderWindow& window){
    ImGui::TextColored(ImVec4(0, 255, 0, 255), "Insert an element into the heap:");
    ImGui::Spacing();
    static char inputBuffer[256] = "";
    bool temp = false;
    ...
    if(checkFinishedV2() && ImGui::Button("Random", ImVec2(100.0f, 30))){
        int value = rand() % 100;
    ...
    if(checkFinishedV2() && (temp || ImGui::Button("Confirm", ImVec2(125.0f, 30)))){
        ...
        scriptV2.push_back({-1, (int)heapList.size(), value, "Đã tạo xong Heap!", StepTypeV2::NEW_NODE});
        scriptV2.push_back({-1, (int)heapList.size(), -1, "Sifting up", StepTypeV2::INSERT});
        insertScriptV2(tempList, value);
        scriptV2.push_back({-1, -1, -1, "Đã tạo xong Heap!", StepTypeV2::FINISH});
    }
}


void updateVisualization2(sf::RenderWindow& window){
    ImGui::TextColored(ImVec4(0, 255, 0, 255), "Update an element in the heap:");
    ImGui::Spacing();
    ...
    if(checkFinishedV2() && ImGui::Button("Random", ImVec2(100.0f, 30)) && heapList.size() > 0) {
    ...
    if (checkFinishedV2() && (temp || ImGui::Button("Confirm", ImVec2(125.0f, 30)))) {
        ...
        if(posStr != "" && valueStr != ""){
            ...
            scriptV2.push_back({2, pos, -1, "Đang duyệt tới vị trí " + std::to_string(pos), StepTypeV2::TRAVERSE});
            scriptV2.push_back({-1, pos, value, "Update", StepTypeV2::UPDATE});
            if(pos > 0){
                if(isMaxHeap){
                    if(std::stoi(tempList[(pos - 1) / 2]->getLabel()) < std::stoi(tempList[pos]->getLabel())) heapifyScriptV2(tempList, pos);
                    else siftDownScriptV2(tempList, pos, tempList.size());
                }
                else{
                    if(std::stoi(tempList[(pos - 1) / 2]->getLabel()) > std::stoi(tempList[pos]->getLabel())) heapifyScriptV2(tempList, pos);
                    else siftDownScriptV2(tempList, pos, tempList.size());
...
void getTopVisualization2(sf::RenderWindow& window){
    ImGui::TextColored(ImVec4(0, 255, 0, 255), "Get the max element in the heap:");
    ImGui::Spacing();
    if(checkFinishedV2() && ImGui::Button("Get", ImVec2(125.0f, 30)) && heapList.size() > 0) {
        scriptV2.clear();
        currentStepIdxV2 = 0;
        std::vector<Block*> tempList = heapList;
        ...
        scriptV2.push_back({-1, 0, -1, "Đã tạo xong Heap!", StepTypeV2::DELETE_1, (int)heapList.size() - 1});
        scriptV2.push_back({-1, 0, -1, "Đã tạo xong Heap!", StepTypeV2::DELETE_2, (int)heapList.size() - 1});
        scriptV2.push_back({-1, 0, -1, "Sifting up", StepTypeV2::MOVE, (int)heapList.size() - 1});
        scriptV2.push_back({-1, 0, -1, "Đã tạo xong Heap!", StepTypeV2::TRAVERSE});
...
void drawHeapList(sf::RenderWindow& window) {
    for (int i=0; i<heapList.size(); i++) {
        heapList[i]->move(dtV2);
        heapList[i]->draw(window);
